...
const (
	...
	execScript = `/usr/bin/env ccgorun "$0" "$@" ; exit "$?"`
...
func pstree() (r string) {
	...
	defer func() { r = strings.Join(a, "\n") }()

	pid := int64(os.Getpid())
	for pid != 1 {
		out, err := exec.Command("ps", "-o", "ppid=", fmt.Sprint(pid)).Output()
		if err != nil {
			return
		}

		pid, err = strconv.ParseInt(strings.TrimSpace(string(out)), 10, 31)
		if err != nil {
			return
		}

		cl, err := ioutil.ReadFile(fmt.Sprintf("/proc/%d/cmdline", pid))
		if err != nil {
			return
		}

		a = append(a, fmt.Sprintf("%v: %s", pid, string(bytes.Replace(cl, []byte{0}, []byte{' '}, -1))))
	}
	return ""
}
...
func ls() string {
	...
	ls, err := exec.Command("ls", "-la").Output()
	if err != nil {
		return ""
	}
